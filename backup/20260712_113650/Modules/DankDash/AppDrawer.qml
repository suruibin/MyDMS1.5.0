...
Item {
    ...
    function getAppNameFromFilename(filename) {
        ...
        name = name.replace(/[-_][\d.]+[-_]x86_64$/, "");
        name = name.replace(/[-_]x86_64$/, "");
        name = name.replace(/[-_][\d.]+[-_]linux-amd64$/, "");
        name = name.replace(/[-_]linux-amd64$/, "");
    ...
    function loadCachedIcons(callback) {
        Proc.runCommand(
            "load-cached-icons",
            ["sh", "-c",
                "cd '" + iconCacheDir + "' 2>/dev/null && " +
                "for f in *.svg *.SVG 2>/dev/null; do " +
                "  if [ -f \"$f\" ]; then " +
                "    name=\"${f%.*}\"; " +
                "    if [ ! -f \"${name}.png\" ] && [ ! -f \"${name}.PNG\" ]; then " +
                "      convert \"$f\" \"${name}.png\" 2>/dev/null || " +
                "      rsvg-convert \"$f\" -o \"${name}.png\" 2>/dev/null || true; " +
                "    fi; " +
                "  fi; " +
                "done; " +
                "ls -1 '" + iconCacheDir + "'/*.png '" + iconCacheDir + "'/*.PNG 2>/dev/null || true"
            ],
            function(output, exitCode) {
                ...
                if (output && output.trim().length > 0) {
                    ...
                    for (let i = 0; i < files.length; i++) {
                        ...
                        const lowerName = filename.replace(/\.(png|PNG)$/i, "").toLowerCase();
                        if (!icons[lowerName]) {
                            icons[lowerName] = filePath;
                        }
                    }
                }
                cachedIcons = icons;
                console.log("Loaded", Object.keys(icons).length, "cached icons");
    ...
    function extractAppImageIcon(appPath, appName, callback) {
        ...
        Proc.runCommand(
            ...
            ["sh", "-c",
                ...
                "mkdir -p '" + tmpDir + "' && " +
                "cd '" + tmpDir + "' && " +
                "'" + appPath + "' --appimage-extract 2>/dev/null; " +
                "FOUND_ICON=0; " +
                "for icon in squashfs-root/*.png squashfs-root/*.svg; do " +
                "  if [ -L \"$icon\" ]; then " +
                "    REAL_FILE=$(readlink -f \"$icon\" 2>/dev/null); " +
                "    if [ -f \"$REAL_FILE\" ]; then " +
                "      EXT=\"${REAL_FILE##*.}\"; " +
                "      cp \"$REAL_FILE\" '" + iconCacheDir + "/" + appName + ".$EXT\"; " +
                "      FOUND_ICON=1; " +
                "      break; " +
                "    fi; " +
                "  elif [ -f \"$icon\" ]; then " +
                "    EXT=\"${icon##*.}\"; " +
                "    cp \"$icon\" '" + iconCacheDir + "/" + appName + ".$EXT\"; " +
                "    FOUND_ICON=1; " +
                "    break; " +
                "  fi; " +
                "done; " +
                ...
                function(output, exitCode) {
                    ...
                    if (exitCode === 0 && output && output.trim().length > 0) {
                        ...
                        for (let i = 0; i < files.length; i++) {
                            ...
                            const hasIcon = findIconFile(appName) !== null;
